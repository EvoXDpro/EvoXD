...
try:
    from pyEvoXD.fns.gDrive import GDriveManager
except ImportError:
    GDriveManager = None
...
from pyEvoXD.fns.helper import fast_download, progress
from pyEvoXD.fns.tools import Carbon, async_searcher, get_paste, telegraph_client
from pyEvoXD.startup.loader import Loader
...
_buttons = {
    "otvars": {
        "text": "Other Variables to set for @EvoXpro:",
        "buttons": [
            [
    ...
    "oofdm": {
        "text": "About [Dual Mode](https://t.me/Evoxdpro/5)",
        "buttons": [
            [
...
async def update(eve):
    ...
    if heroku_api:
        ...
        execl(sys.executable, sys.executable, "-m", "pyEvoXD")
...
async def changes(okk):
    ...
    if not match:
        try:
            ...
            return await okk.edit(
                f"**• EvoXD Userbot •**{cli}", file=img, buttons=button
            )
        except Exception as er:
            LOGS.exception(er)
    ...
    if len(changelog_str) > 1024:
        ...
        with open("EvoXD_updates.txt", "w+") as file:
...
async def _(e):
    ...
    async with asst.conversation(e.sender_id) as conv:
        ...
        if GDrive._create_token_file(code=code.text):
            await conv.send_message(
                "`Success!\nYou are all set to use Google Drive with EvoXD Userbot.`",
...
async def _(e):
    if not e.is_private:
        return
    await e.edit(
        "Click Authorise and send the code.\n\nYou can use your own CLIENT ID and SECRET by [this](https://t.me/Evoxdpro/6)",
        buttons=[
            [
