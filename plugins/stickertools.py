...
async def pack_kangish(_):
    ...
    _packname = cmdtext or f"EvoXD Kang Pack By {_.sender_id}"
    typee = None
    if not local:
...
async def hehe(args):
    ...
    if message.photo:
        photo = io.BytesIO()
        photo = await EvoXD_bot.download_media(message.photo, photo)
    elif message.file and "image" in message.file.mime_type.split("/"):
        photo = io.BytesIO()
        await EvoXD_bot.download_file(message.media.document, photo)
        if (
    ...
    if photo:
        ...
        if not emoji:
            emoji = "🏵"
        if len(splat) == 3:
            pack = splat[2]  # User sent EvoXD_both
            emoji = splat[1]
        elif len(splat) == 2:
        ...
        if (
            ...
            async with EvoXD_bot.conversation("@Stickers") as conv:
                try:
                    await conv.send_message("/addsticker")
                except YouBlockedUserError:
                    LOGS.info("Unblocking @Stickers for kang...")
                    await EvoXD_bot(functions.contacts.UnblockRequest("stickers"))
                    await conv.send_message("/addsticker")
                ...
                await conv.get_response()
                await EvoXD_bot.send_read_acknowledge(conv.chat_id)
        else:
...
async def ultdestroy(event):
    ...
    await event.client.download_media(ult, "EvoXD.tgs")
    xx = await event.eor(get_string("com_1"))
    await bash("lottie_convert.py EvoXD.tgs json.json")
    ...
    file = await con.animated_sticker("json.json", "EvoXD.tgs")
    if file:
        await event.client.send_file(
            event.chat_id,
            file="EvoXD.tgs",
...
async def ultiny(event):
    ...
    im1 = Image.open("resources/extras/EvoXD_blank.png")
    if ik.endswith(".tgs"):
