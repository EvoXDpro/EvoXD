...
from pyEvoXD.fns.info import get_uinfo

from . import HNDLR, LOGS, eod, eor, get_string, inline_mention, types, puii_cmd
...
@EvoXD_cmd(pattern="pin$", manager=True, require="pin_messages", fullsudo=True)
async def pin(msg):
    if not msg.is_reply:
...
@EvoXD_cmd(pattern="purge( (.*)|$)", manager=True, require="delete_messages")
...
@EvoXD_cmd(pattern="pinned", manager=True, groups_only=True)
