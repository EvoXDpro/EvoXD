...
from . import get_string, udB, EvoXD_bot, EvoXD_cmd

keym = KeyManager("NIGHT_CHATS", cast=list)


@EvoXD_cmd(pattern="nmtime( (.*)|$)")
...
@EvoXD_cmd(pattern="addnm( (.*)|$)")
async def add_grp(e):
    if pat := e.pattern_match.group(1).strip():
        try:
            keym.add((await EvoXD_bot.get_entity(pat)).id)
...
@EvoXD_cmd(pattern="remnm( (.*)|$)")
async def r_em_grp(e):
    if pat := e.pattern_match.group(1).strip():
        try:
            keym.remove((await EvoXD_bot.get_entity(pat)).id)
...
@EvoXD_cmd(pattern="listnm$")
async def rem_grp(e):
    chats = keym.get()
    name = "NightMode Groups Are-:\n\n"
    for x in chats:
        try:
            ok = await EvoXD_bot.get_entity(x)
...
async def open_grp():
    for chat in keym.get():
        try:
            ...
            await EvoXD_bot.send_message(chat, "**NightMode Off**\n\nGroup Opened 🥳.")
